...
def gen_pxe(new):
	# type: (dict) -> str
	args = [configRegistry.get('pxe/installer/append')]
	if args[0] is None:
		...
		vga = configRegistry.get("pxe/installer/vga")
		args += [
		...
		args += [
			# Kernel
			'quiet' if configRegistry.is_true('pxe/installer/quiet', False) else '',
			'loglevel=%s' % configRegistry.get('pxe/installer/loglevel', '0'),
			# Debian installer
			'auto-install/enable=true',
			'preseed/url=%s' % (url,),
			'mirror/http/hostname=%s' % (configRegistry.get("repository/online/server", FQDN),),
			# 'DEBCONF_DEBUG=5',
		]
	...
	return dedent('''\
			...
			LABEL local
				LOCALBOOT 0
			''') % {
		'kernel': configRegistry.get('pxe/installer/kernel', 'linux'),
		'initrd': configRegistry.get('pxe/installer/initrd', 'initrd.gz'),
		'append': append,
		'ipappend': configRegistry.get('pxe/installer/ipappend', '0'),
	}
