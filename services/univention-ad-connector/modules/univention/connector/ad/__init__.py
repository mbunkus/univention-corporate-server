...
# mapping funtions
def samaccountname_dn_mapping(connector, given_object, dn_mapping_stored, ucsobject, propertyname, propertyattrib, ocucs, ucsattrib, osc4, dn_attr=None):
	'''
	map dn of given object (which must have an samaccountname in AD)
	ocucs and osc4 are objectclasses in UCS and AD
	'''
	...
	for dn_key in ['dn', 'olddn']:
		...
		if dn_key in object and not dn_premapped(object, dn_key, dn_mapping_stored):
			...
			if ucsobject:
				...
				filter_parts_ad = [format_escaped('(objectclass={0!e})', osc4), ]
				...
				result = connector.ad_search_ext_s(connector.lo_ad.base, ldap.SCOPE_SUBTREE, filter_ad, ['sAMAccountName'])
				...
				while (not samaccountname):  # in case of olddn this is already set
					...
					try:
						samaccountname_filter = format_escaped('(objectClass={0!e})', osc4)
...
def dc_dn_mapping(connector, given_object, dn_mapping_stored, isUCSobject):
	'''
	map dn of given dc computer using the samaccountname/uid
	connector is an instance of univention.connector.ad, given_object an object-dict,
	dn_mapping_stored a list of dn-types which are already mapped because they were stored in the config-file
	'''
	return samaccountname_dn_mapping(connector, given_object, dn_mapping_stored, isUCSobject, 'dc', u'samAccountName', u'posixAccount', 'uid', u'computer', 'cn')
...
class ad(univention.connector.ucs):
	...
	def main(cls, ucr=None, configbasename='connector', **kwargs):
		...
		try:
			...
			ad_ldap_binddn = _ucr.get('%s/ad/ldap/binddn' % configbasename, None)
			ad_ldap_certificate = _ucr.get('%s/ad/ldap/certificate' % configbasename)
			if not ad_ldap_certificate and ucr.is_true('%s/ad/ldap/ssl' % configbasename):
				raise KeyError('%s/ad/ldap/certificate' % configbasename)
			listener_dir = _ucr['%s/ad/listener/dir' % configbasename]
		except KeyError as exc:
			raise SystemExit('UCR variable %s is not set' % (exc,))

		ad_ldap_bindpw = None
		if ucr.get('%s/ad/ldap/bindpw' % configbasename) and os.path.exists(ucr['%s/ad/ldap/bindpw' % configbasename]):
			with open(ucr['%s/ad/ldap/bindpw' % configbasename]) as fd:
	...
	def __init__(self, CONFIGBASENAME, property, configRegistry, ad_ldap_host, ad_ldap_port, ad_ldap_base, ad_ldap_binddn, ad_ldap_bindpw, ad_ldap_certificate, listener_dir, logfilename=None, debug_level=None):
		...
		if 'univention_samaccountname_ldap_check' in self.configRegistry.get('samba4/ldb/sam/module/prepend', '').split():
			# The AD connector must bypass this LDB module if it is activated via samba4/ldb/sam/module/prepend
		...
		# objectSid modification for an AD object is only possible with the "provision" control:
		if self.configRegistry.is_true('connector/ad/mapping/sid_to_ad', False):
		...
		# five users are created on UCS side. After two users have been synced to AD/AD,
		# the group is snyced. But in AD/AD only existing members can be stored in the group.
		# Now the sync goes back from AD/AD to LDAP and we should not remove the three users
	...
	def open_ad(self):
		...
		try:
			self.lo_ad = univention.uldap.access(host=self.ad_ldap_host, port=int(self.ad_ldap_port), base='', binddn=None, bindpw=None, start_tls=tls_mode, ca_certfile=self.ad_ldap_certificate, uri=ldapuri, reconnect=False)
		...
		self.lo_ad = univention.uldap.access(host=self.ad_ldap_host, port=int(self.ad_ldap_port), base=self.ad_ldap_base, binddn=self.ad_ldap_binddn, bindpw=self.ad_ldap_bindpw, start_tls=tls_mode, ca_certfile=self.ad_ldap_certificate, decode_ignorelist=DECODE_IGNORELIST, uri=ldapuri, reconnect=False)

		self.lo_ad.lo.set_option(ldap.OPT_REFERRALS, 0)

		if self.configRegistry.get('connector/ad/mapping/dns/position') == 'legacy':
			self.ad_ldap_partitions = (self.ad_ldap_base,)
		else:
			self.ad_ldap_partitions = (self.ad_ldap_base, "DC=DomainDnsZones,%s" % self.ad_ldap_base, "DC=ForestDnsZones,%s" % self.ad_ldap_base)
	...
	def get_object_dn(self, dn):
		for i in [0, 1]:  # do it twice if the LDAP connection was closed
			try:
				dn, ad_object = self.ad_search_ext_s(dn, ldap.SCOPE_BASE, '(objectClass=*)', ('dn',))[0]
	...
	def get_object(self, dn, attrlist=None):
		...
		for i in [0, 1]:  # do it twice if the LDAP connection was closed
			try:
				dn, ad_object = self.ad_search_ext_s(dn, ldap.SCOPE_BASE, '(objectClass=*)', attrlist=attrlist)[0]
				ud.debug(ud.LDAP, ud.INFO, "get_object: got object: %r" % (dn,))
				return ad_object
			except (IndexError, ldap.NO_SUCH_OBJECT):
				return
	...
	def object_memberships_sync_from_ucs(self, key, object):
		...
		if 'group' in self.property:
			if getattr(self.property['group'], 'sync_mode', '') in ['read', 'none']:
				ud.debug(ud.LDAP, ud.INFO, "group memberships sync to ad ignored, group sync_mode is read")
				return
	...
	def group_members_sync_from_ucs(self, key, object):  # object mit ad-dn
		...
		# lookup all current members of AD group
		ldap_object_ad = self.get_object(object['dn'])
		if not ldap_object_ad:
		...
		for member_dn in ucs_members:
			...
			if ad_dn:
				ud.debug(ud.LDAP, ud.INFO, "Found %s in UCS group member cache: %s" % (member_dn, ad_dn))
		...
		if add_members or del_members:
			ad_members |= add_members  # Note: add_members are only lowercase
			ad_members -= del_members  # Note: del_members are case sensitive
			ud.debug(ud.LDAP, ud.INFO, "group_members_sync_from_ucs: members result: %r" % ad_members)

			self.lo_ad.lo.modify_s(object['dn'], [(ldap.MOD_REPLACE, 'member', [x.encode('UTF-8') for x in ad_members])])
	...
	def group_members_sync_to_ucs(self, key, object):
		...
		for member_dn in ucs_members:
			member_dn_lower = member_dn.lower()
			if not (member_dn_lower in ucs_members_from_ad['user'] or member_dn_lower in ucs_members_from_ad['group'] or member_dn_lower in ucs_members_from_ad['unknown']):
				try:
	...
	def sync_from_ucs(self, property_type, object, pre_mapped_ucs_dn, old_dn=None, old_ucs_object=None, new_ucs_object=None):
		...
		if ad_object:
			objectGUID = univention.connector.decode_guid(ad_object.get('objectGUID')[0])
			if self.lockingdb.is_ad_locked(objectGUID):
		...
		try:
			...
			entryUUID = None  # may be empty for back_mapped_subobject for leaf object delete_in_ad
		...
		if not ad_object and object['modtype'] in ('add', 'modify', 'move'):
			...
			if hasattr(self.property[property_type], "con_sync_function"):
				...
				try:
					...
					filter_ad = format_escaped(u'(&(sAMAccountName={0!e})(objectSid={1!e})(isDeleted=TRUE))', sAMAccountName, sambaSID)
					ud.debug(ud.LDAP, ud.PROCESS, "sync_from_ucs: error during add, searching for conflicting deleted object in AD")
					ud.debug(ud.LDAP, ud.INFO, "sync_from_ucs: search filter: %s" % filter_ad)
					result = self.ad_search_ext_s(self.lo_ad.base, ldap.SCOPE_SUBTREE, filter_ad, ['dn'], serverctrls=[LDAPControl(
					...
					self.lo_ad.lo.modify_ext_s(result[0][0], reanimate_modlist, serverctrls=[LDAPControl(LDAP_SERVER_SHOW_DELETED_OID, criticality=1), ])
							...
							for attribute in attribute_type.keys():
								...
								if ad_other_attribute:
									...
									if (to_add or to_remove) and (attribute_type[attribute].single_value or has_mapping_function):
										modified = (not current_ad_values or not value) or \
											not attribute_type[attribute].compare_function(list(current_ad_values), list(value))
										if modified:
